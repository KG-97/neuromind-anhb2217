...
const NeuronModel: React.FC = () => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
      <div className="lg:col-span-2 glass-panel p-6 flex flex-col items-center justify-center relative overflow-hidden rounded-2xl border-none">
        <h3 className="absolute top-4 left-4 text-xl font-bold text-zinc-50 flex items-center gap-2">
          <Activity className="w-5 h-5 text-violet-500" />
          Interactive Neuron Model
        </h3>
        <p className="absolute top-12 left-4 text-sm text-zinc-400">Click the components to learn more.</p>
      ...
      <div className="glass-panel rounded-2xl p-6 flex flex-col border-none">
        {selectedPart && parts[selectedPart as keyof typeof parts] ? (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex items-center gap-3 mb-4">
              <div className="p-3 glass-button rounded-lg text-violet-400 border-none">
                <Info size={24} />
              </div>
              <h2 className="text-2xl font-bold text-zinc-50">{parts[selectedPart as keyof typeof parts].title}</h2>
            </div>
            <p className="text-zinc-300 leading-relaxed text-lg">
              {parts[selectedPart as keyof typeof parts].desc}
            </p>
            <div className="mt-8 p-4 glass-card rounded-xl border-none shadow-sm">
